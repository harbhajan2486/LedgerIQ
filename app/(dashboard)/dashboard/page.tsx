    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={stats?.todayDocs ?? 0}
          icon={<FileText size={18} className="text-blue-500" />}
          href="/upload"
          emptyHint="Upload your first document"
        />
        <StatCard
          ...
          href="/review"
          badge={stats?.pendingReview ? { label: "Action needed", variant: "destructive" } : undefined}
        />
        <StatCard
          title="Matched This Week"
          value={stats?.matchedThisWeek ?? 0}
        ...
        <StatCard
          ...
          href="/reconciliation"
          badge={stats?.exceptions ? { label: "Review needed", variant: "destructive" } : undefined}
        />
      </div>
        ...
        <Card className="border-dashed border-2 border-gray-200">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-sm mb-6">
              LedgerIQ will read your invoices, map GST and TDS, and match them
              to your bank transactions automatically.
            </p>
              ...
              {stats.recentActivity.map((item, i) => (
                ...
                >
                  <span className="text-gray-700 capitalize">
                    {item.action.replace(/_/g, " ")} — {item.entity_type}
                  </span>
...
function StatCard({
  ...
  href: string;
  badge?: { label: string; variant: "destructive" | "secondary" };
  emptyHint?: string;
}) {
  return (
    <Link href={href}>
      <Card className="hover:shadow-md transition-shadow cursor-pointer h-full">
        <CardContent className="pt-5 pb-4">
          ...
          <div className="flex items-end gap-2">
            ...
            {badge && value > 0 && (
              <Badge variant={badge.variant} className="mb-1 text-xs">
                {badge.label}
              </Badge>
            )}
          </div>
          {value === 0 && emptyHint && (
            <p className="text-xs text-blue-500 mt-1">{emptyHint}</p>
          )}
        </CardContent>
      </Card>
    </Link>
